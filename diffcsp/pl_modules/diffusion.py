...
class CSPDiffusion(BaseModule):
    ...
    @torch.no_grad()
    def sample_new_method_and_random(self, batch, step_lr = 1e-5):
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            if t < 1000:
                ...
                dellogp_delx_t, m, c = calculate_dellogp_delx_t_with_all_flow(x_t_new, pred_x_new, pred_x_d2, sigma_x, batch, fq)

                # x_t_minus_1_new = x_t_new - step_size * (pred_x_new + 3 * dellogp_delx_t) + std_x * rand_x if not self.keep_coords else x_t_new
                x_t_minus_1_new = x_t_new - step_size * (pred_x_new + 3 * dellogp_delx_t) if not self.keep_coords else x_t_new
            else:
                m = x_t_new
                c = torch.full(pred_x.shape, 0.01, device=pred_x.device)
