...
def optimize_mc(x_t, sigma, target1, target2, lr=0.01, iterations=1000):
    ...
    for _ in range(iterations):
        ...
        # a_n, b_n の計算
        a_n_values = torch.tensor([compute_fourier_an(n, sigma) for n in range(1, 6)], dtype=torch.float32, requires_grad=True).view(-1, 1, 1)
        b_n_values = torch.tensor([compute_fourier_bn(n, sigma) for n in range(0, 6)], dtype=torch.float32, requires_grad=True).view(-1, 1, 1)
