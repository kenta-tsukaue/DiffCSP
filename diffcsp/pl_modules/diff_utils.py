...
from diffcsp.pl_modules.crystal_utils import complex_sum_squared_with_scattering_factors, complex_sum_squared_with_scattering_factors_torch,  calculate_q_magnitude, scattering_factor, initialize_coefficients, calculate_delI_delm_delx_t_vectorized, calculate_delI_delc_delx_t_vectorized, calculate_y_vectorized_re, calculate_I_vectorized_re
...
def find_best_fit_optimized(s1_table, s2_table, m_table, c_table, score1, score2, sigma):
    # すべてのテンソルを同じデバイスに移動
    device = sigma.device
    s1_table = torch.tensor(s1_table, device=device) if not isinstance(s1_table, torch.Tensor) else s1_table.to(device)
    s2_table = torch.tensor(s2_table, device=device) if not isinstance(s2_table, torch.Tensor) else s2_table.to(device)
    m_table = torch.tensor(m_table, device=device) if not isinstance(m_table, torch.Tensor) else m_table.to(device)
    c_table = torch.tensor(c_table, device=device) if not isinstance(c_table, torch.Tensor) else c_table.to(device)
    ...
    # スコアを拡張して計算
    sigma2_score1 = sigma2 * score1[:, :, None, None]
    sigma4_score2 = sigma4 * score2[:, :, None, None]
    ...
    min_indices = torch.argmin(err, dim=2)  # 各 (n, d) について最小のインデックス

    # インデックスから m と c の値を取得
    kmin = min_indices // c_len
    lmin = min_indices % c_len
    m = m_table[kmin]  # (n, d)
    c = c_table[lmin]  # (n, d)

    return m.cpu(), c.cpu()

def find_best_fit_optimized_2(s1_table, s2_table, m_table, c_table, score1, score2, sigma):
    # すべてのテンソルを同じデバイスに移動
    device = sigma.device
    s1_table = torch.tensor(s1_table, device=device) if not isinstance(s1_table, torch.Tensor) else s1_table.to(device)
    s2_table = torch.tensor(s2_table, device=device) if not isinstance(s2_table, torch.Tensor) else s2_table.to(device)
    ...
    n, d, m_len, c_len = s1_table.shape
    total_combinations = m_len * c_len

    # テーブルをフラット化
    s1_table_flat = s1_table.view(n, d, -1)  # 形状 (n, d, total_combinations)
    s2_table_flat = s2_table.view(n, d, -1)  # 形状 (n, d, total_combinations)
    ...
    # エラーの最小値のインデックスを取得
    min_indices = torch.argmin(err, dim=2)  # 形状 (n, d)

    # インデックスから m と c の値を取得
    kmin = min_indices // c_len
    lmin = min_indices % c_len
    m = m_table[kmin]  # 形状 (n, d)
    c = c_table[lmin]  # 形状 (n, d)

    return m.cpu(), c.cpu()


# 条件スコアを算出
def calculate_dellogp_delx_t(I, y, delI, num_atoms, sigma=0.5, epsilon=1e-8):
    # I, y: (n, 5, 5, 5)
    # delI: (n, 5, 5, 5, m, 3)
    ...
    difference = (I - y)[:, :, :, :, np.newaxis, np.newaxis]  # Shape: (n, 5, 5, 5, 1, 1)
...
def calculate_loss(batch, traj, c, fq):
    batch_y = calculate_y_vectorized_re(batch, c, fq)
    traj_y = calculate_y_vectorized_re(traj, c, fq)
...
def calculate_loss_unit(batch, traj, c, fq):
    batch_y = calculate_y_vectorized_re(batch, c, fq)
    traj_y = calculate_y_vectorized_re(traj, c, fq)
    ...
    # バッチごとにRMSEを計算する
    rmse_list = []
    batch_size = batch_y.shape[0]  # 256
    
    for i in range(batch_size):
        mse = torch.nn.functional.mse_loss(batch_y[i], traj_y[i])
        rmse = torch.sqrt(mse)
...
def calculate_delx_t_kari(m1, m2, c1, c2, s1, s2, regularization=1e-8):
    m1, m2, c1, c2, s1, s2 = map(to_cpu, [m1, m2, c1, c2, s1, s2])
    ...
    for i in range(n):
        for j in range(3):
            ...
            if np.abs(det) > 1e-12:
                ...
                # 行列式がゼロの場合、最小二乗法で解を求める
                solution, residuals, rank, s = np.linalg.lstsq(M, C, rcond=None)
                # または、正則化を加える方法
                # M_reg = M + regularization * np.eye(2)
                # solution = np.linalg.solve(M_reg, C)
...
def scale_dellogp(dellogp_delx_t, pred_x, num_atoms, epsilon=1e-8):
    # num_atoms をリストに変換
    ...
    for dellogp, pred in zip(split_dellogp_delx_t, split_pred_x):
        ...
        scaling_factors = max_abs_values_pred / (max_abs_values_dellogp + epsilon)
...
def calculate_dellogp_delx_t_with_all_flow(x_t, pred_x, pred_x_d2, sigma_x, batch, fq):
    ...
    # y = calculate_y_vectorized(batch, c)
    y = calculate_y_vectorized_re(batch, c, fq)
    check_nan_inf(y, "y")
    #log_with_timestamp("3")
    
    # m, cから予測される構造因子を求める
    # I = calculate_I(batch, m, c)
    # I = calculate_I_vectorized(batch, m, c)
    I = calculate_I_vectorized_re(batch, m, c, fq)
    check_nan_inf(I, "I")
    ...
    check_nan_inf(del1_delm, "del1_delm")
    #log_with_timestamp("6")
    del1_delc = calculate_del1_delc(m, c, sigma_x, x_t)
    check_nan_inf(del1_delc, "del1_delc")
    #log_with_timestamp("7")

    # ∂(2)/∂m, ∂(2)/∂cを求める
    del2_delm = calculate_del2_delm(m, c, sigma_x, x_t)
    check_nan_inf(del2_delm, "del2_delm")
    #log_with_timestamp("8")
    del2_delc = calculate_del2_delc(m, c, sigma_x, x_t)
    check_nan_inf(del2_delc, "del2_delc")
    #log_with_timestamp("9")

    # ∂m/∂x_t, ∂c/x_tを求める
    delm_delx, delc_delx = calculate_delx_t_kari(del1_delm, del2_delm, del1_delc, del2_delc, pred_x, pred_x_d2)
    check_nan_inf(delm_delx, "delm_delx")
    check_nan_inf(delc_delx, "delc_delx")
    #log_with_timestamp("10")

    # ∂I/∂x_tを求める
    #delI_delm_delm_delx_t = calculate_delI_delm_delm_delx_t(batch.num_atoms, batch, m, c, delm_delx, 2)
    delI_delm_delm_delx_t = calculate_delI_delm_delx_t_vectorized(batch, m, c, delm_delx, fq)
    check_nan_inf(delI_delm_delm_delx_t, "delI_delm_delm_delx_t,")
    
    #print(delI_delm_delm_delx_t)
    #log_with_timestamp("11")
    # delI_delc_delc_delx_t = calculate_delI_delc_delc_delx_t(batch.num_atoms, batch, m, c, delc_delx, 2)
    delI_delc_delc_delx_t = calculate_delI_delc_delx_t_vectorized(batch, m, c, delc_delx, fq)
    check_nan_inf(delI_delc_delc_delx_t, "delI_delc_delc_delx_t")
    #log_with_timestamp("12")

    delI_delx_t = delI_delm_delm_delx_t + delI_delc_delc_delx_t
    upper_limit = 1e10  # 例: 上限値を1e10に設定
    delI_delx_t = np.nan_to_num(delI_delx_t, posinf=upper_limit, neginf=-upper_limit, nan=0.0)
    check_nan_inf(delI_delx_t, "dellogp_delx_t_1")
...
def generate_crystal_structures_4():
    scale_factor = 0.6
    offset = 0.5 * (1 - scale_factor)
    ...
    line_2 = [[0.5, 0.5, (i + 4) * 0.125] for i in range(-2, 3)]
    line_2 = [[x * scale_factor + offset, y * scale_factor + offset, z] for x, y, z in line]

    

    structures = [line[:5], line_2[:5]] * 5000
...
def generate_crystal_Cu3Au():
    ...
    # Cu₃Auの超格子構造（固定の構造）
    frac_coords_superlattice = [
        [0.5, 0.5, 0.5],  # Cu1
        [0.0, 0.0, 0.5],  # Cu2
        [0.5, 0.0, 0.0],  # Cu3
        [0.0, 0.5, 0.0],  # Au (規則的に配置)
    ]

    # 超格子構造に使うCuとAuの原子番号
    atom_types_superlattice = [29, 29, 29, 79]

    # ランダム配置用のCu₃Au
    def generate_random_frac_coords():
        # Cu原子の位置は固定
        frac_coords_random = [
            [0.5, 0.5, 0.5],  # Cu1
            [0.0, 0.0, 0.5],  # Cu2
        ...
        frac_coords_random.append(au_random_coords)
        return frac_coords_random
...
def generate_crystal_Cu24Au8(convert=True):
    # 10000個の結晶構造を格納するリスト
    structures = []

    au_regular_coords = [
        [0.0, 0.0, 0.25], [0.25, 0.0, 0.0], [0.0, 0.25, 0.0], [0.5, 0.0, 0.25],
        [0.75, 0.0, 0.0], [0.5, 0.25, 0.0], [0.0, 0.5, 0.25], [0.25, 0.5, 0.0],
        [0.0, 0.75, 0.0], [0.5, 0.5, 0.25], [0.75, 0.5, 0.0], [0.5, 0.75, 0.0],
        [0.0, 0.0, 0.75], [0.25, 0.0, 0.5], [0.0, 0.25, 0.5], [0.5, 0.0, 0.75],
        [0.75, 0.0, 0.5], [0.5, 0.25, 0.5], [0.0, 0.5, 0.75], [0.25, 0.5, 0.5],
        [0.0, 0.75, 0.5], [0.5, 0.5, 0.75], [0.75, 0.5, 0.5], [0.5, 0.75, 0.5],
...
def calculate_y_vectorized(batch, c):
    """
    バッチ内の全ての結晶に対して回折強度を計算する関数。
    """
    num_crystals = batch['num_atoms'].size(0)  # バッチサイズ
    device = batch['frac_coords'].device
    k_values = torch.arange(-2, 3, device=device, dtype=batch['frac_coords'].dtype)
    k_grid = torch.stack(torch.meshgrid(k_values, k_values, k_values, indexing='ij'), dim=-1).reshape(-1, 3)  # shape (125, 3)
    Z = torch.zeros((num_crystals, 5, 5, 5), device=device)
    a_coeff, b_coeff, c_coeff = initialize_coefficients(device='cuda:0')
    ...
    for I in range(num_crystals):
        ...
        # A_c を (n_atoms, 3) に変換
        if A_c.dim() == 1:
            # c[I] が (3,) の場合、全ての原子に対して同じ補正を適用
            if A_c.size(0) == 3:
                A_c = A_c.unsqueeze(0).expand(first_frac_coords.size(0), -1)  # (n_atoms, 3)
            else:
                raise ValueError(f"Unexpected size for c[I]: {A_c.size()}")

        # PyTorch対応の関数を使用
        results = complex_sum_squared_with_scattering_factors_torch(
            k_grid, first_frac_coords, A_c, atom_types, a_coeff, b_coeff, c_coeff
        )  # shape (125,)
        Z[I] = results.reshape(5, 5, 5)

    return Z.cpu().numpy()


def calculate_I_vectorized(batch, m, c):
    """
    バッチ内の全ての結晶に対して回折強度を計算する関数。
    """
    num_crystals = batch['num_atoms'].size(0)  # バッチサイズ
    ...
    for I in range(num_crystals):
        ...
        if A_c.dim() == 1:
            # c[I] が (3,) の場合、全ての原子に対して同じ補正を適用
            if A_c.size(0) == 3:
                A_c = A_c.unsqueeze(0).expand(frac_coords_m.size(0), -1)  # (n_atoms, 3)
            else:
                raise ValueError(f"Unexpected size for c[I]: {A_c.size()}")

        # PyTorch対応の関数を使用
        results = complex_sum_squared_with_scattering_factors_torch(
            k_grid, frac_coords_m, A_c, atom_types, a_coeff, b_coeff, c_coeff
        )  # shape (125,)
        Z[I] = results.reshape(5, 5, 5)
